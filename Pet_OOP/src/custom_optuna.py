...
class CustomOptuna:
    ...
    class Objective:
        ...
        refit : bool
            Флаг, для указания необходимости обучения модели.
        ...
        def __init__(self, model, X, y, hpspace, refit, cv, scorer, random_state, is_pipeline):
            """
        ...
        def BestModelCallback(self, study, trial):
            ...
            if study.best_trial.number == trial.number:
                if self.refit:
                    self.best_model_ = deepcopy(self.model.fit(self.X, self.y))
                else:
    ...
    def study_optimize(self, model, X, y, hpspace=None, refit=True):
        """
        ...
        self.objective_to_study = self.Objective(optuna_model, X, y, self.hpspace, refit, self.cv, self.scorer, self.random_state, self.is_pipeline)
